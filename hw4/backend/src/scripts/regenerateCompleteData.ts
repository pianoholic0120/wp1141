...
const allAmenities = [
  ...
  'management_fee', 'utilities_included', 'floor', 'near_shopping', 'near_hospital',
  'furnished', 'concierge', 'elevator', 'parking', 'balcony', 'garden',
  'swimming_pool', 'tennis_court', 'library', 'study_room', 'laundry_room',
  'storage_room', 'bike_storage', 'mail_room', 'lobby', 'reception'
];

// 生成房東資料
function generateLandlord() {
  const surnames = ['王', '李', '張', '劉', '陳', '楊', '黃', '趙', '周', '吳', '徐', '孫', '馬', '朱', '胡', '郭', '何', '高', '林', '羅'];
  const givenNames = ['志明', '淑芬', '建國', '美玲', '家豪', '雅婷', '志強', '淑娟', '文雄', '麗華', '俊傑', '雅芳', '志偉', '淑惠', '明輝', '雅玲', '建華', '淑華', '文華', '美華'];
...
// 生成完整的房屋資料
function generateCompleteListing(landlordId: number) {
  const location = taiwanLocations[Math.floor(Math.random() * taiwanLocations.length)];
  const propertyType = propertyTypes[Math.floor(Math.random() * propertyTypes.length)];
  const status = statuses[Math.floor(Math.random() * statuses.length)];
  
  // 在選定區域內生成精確的座標
  const lat = location.lat + (Math.random() - 0.5) * 0.005; // ±0.005 度
  const lng = location.lng + (Math.random() - 0.5) * 0.005; // ±0.005 度
  ...
  // 生成完整的公設（5-15個公設）
  const amenityCount = Math.floor(Math.random() * 11) + 5; // 5-15 個公設
...
async function regenerateCompleteData() {
  console.log('🚀 開始完全重新生成400筆房屋資料...');
  ...
  try {
    ...
    for (let i = 0; i < 400; i++) {
      ...
      const listing = generateCompleteListing(landlordId);
      ...
      console.log(`✅ 已創建房屋: ${listing.title} (ID: ${result.lastInsertRowid}) - ${listing.address} (${listing.latitude.toFixed(4)}, ${listing.longitude.toFixed(4)}) - 公設: ${listing.amenities.length}個`);
    }
    
    console.log('🎉 完整資料重新生成完成！');
    console.log(`📊 統計:`);
    console.log(`   - 房東數量: ${landlords.length}`);
    console.log(`   - 房屋數量: 400`);
    ...
    console.log(`   - 公設選項: ${allAmenities.length} 種完整公設`);
    
  } catch (error) {
    console.error('❌ 重新生成資料時發生錯誤:', error);
    throw error;
  }
}
...
if (require.main === module) {
  regenerateCompleteData().catch(console.error);
}

export { regenerateCompleteData };
