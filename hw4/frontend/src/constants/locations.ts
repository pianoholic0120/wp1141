...
export const TAIWAN_LOCATIONS: Location[] = [
  {
    city: '台北市',
    districts: [
  ...
  },
  {
    city: '花蓮縣',
    districts: [
      '花蓮市', '新城鄉', '太魯閣', '秀林鄉', '吉安鄉', '壽豐鄉',
      '鳳林鎮', '光復鄉', '豐濱鄉', '瑞穗鄉', '玉里鎮', '卓溪鄉',
      '富里鄉'
    ]
  },
  {
...
export function getCities(): string[] {
  return TAIWAN_LOCATIONS.map(location => location.city);
}

// 根據縣市獲取區域列表
export function getDistrictsByCity(city: string): string[] {
  const location = TAIWAN_LOCATIONS.find(loc => loc.city === city);
  return location ? location.districts : [];
}

// 檢查縣市是否存在
export function isValidCity(city: string): boolean {
  return TAIWAN_LOCATIONS.some(loc => loc.city === city);
}
