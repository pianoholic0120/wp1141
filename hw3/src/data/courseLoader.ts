...
// CSV format: day1, day2, day3, day4, day5, day6 contain "節次代號"
// Examples: day1="12" (Monday, periods 1,2), day4="567" (Thursday, periods 5,6,7)
const parseTimeSlots = (timeValue: string | undefined, dayIndex: number): TimeSlot[] => {
  const timeSlots: TimeSlot[] = [];
  if (!timeValue || timeValue.trim() === '') return timeSlots;

  // NTU day mapping: 1=Mon, 2=Tue, 3=Wed, 4=Thu, 5=Fri, 6=Sat, 7=Sun
  const dayMap: { [key: number]: 'Mon' | 'Tue' | 'Wed' | 'Thu' | 'Fri' | 'Sat' | 'Sun' } = {
    1: 'Mon', 2: 'Tue', 3: 'Wed', 4: 'Thu', 5: 'Fri', 6: 'Sat', 7: 'Sun'
  };

  // NTU time slot mapping - 根據用戶提供的對照表
  const timeSlotMap: { [key: string]: { start: number; end: number } } = {
    ...
    'A': { start: 18, end: 19 }, // 18:25 - 19:15
    'B': { start: 19, end: 20 }, // 19:20 - 20:10
    'C': { start: 20, end: 21 }, // 20:15 - 21:05
    'D': { start: 21, end: 22 }  // 21:10 - 22:00
  };

  // Get day from dayIndex
  const day = dayMap[dayIndex];
  if (!day) return timeSlots;

  // Parse time string (e.g., "12" -> ['1', '2'] or "567" -> ['5', '6', '7'])
  const timeCodes = timeValue.trim().split('').filter(t => timeSlotMap[t]);
      ...
      .then(csvText => {
        Papa.parse(csvText, {
          ...
          complete: (results) => {
              ...
              // Iterate through day columns (day1, day2, day3, day4, day5, day6)
              for (let i = 1; i <= 6; i++) {
                const dayKey = `day${i}`;
                const dayValue = row[dayKey];
                
                if (dayValue && dayValue.trim() !== '') {
                  const parsedSlots = parseTimeSlots(dayValue, i);
                  if (parsedSlots.length > 0) {
                    console.log(`Row ${index}: ${dayKey}=${dayValue} -> ${parsedSlots.length} time slots`);
              ...
              return {
                ...
                year: ['1', '2', '3', '4'].includes(row.year) ? row.year : '1',
                ...
                enrolled: parseInt(row.tno || '0')
                    ...
                    // Sort courses by department and course name for better organization
  ...
  const filtered = courses.filter(course => {
    ...
    if (filters.department && filters.department.length > 0) {
      if (!filters.department.includes(course.department)) {
        console.log(`Course ${course.cou_cname} filtered out by department: ${course.department} not in ${filters.department}`);
        return false;
      }
    }
    ...
    if (filters.year && filters.year.length > 0) {
      if (!filters.year.includes(course.year)) {
        console.log(`Course ${course.cou_cname} filtered out by year: ${course.year} not in ${filters.year}`);
        return false;
      }
    }
    ...
    if (filters.credits && filters.credits.length > 0) {
      if (!filters.credits.includes(course.credits)) {
        console.log(`Course ${course.cou_cname} filtered out by credits: ${course.credits} not in ${filters.credits}`);
        return false;
      }
    }
